...
describe('jurisdiction_risk.json', () => {
  ...
  it('high risk has purpose-built authority on multiple axes', () => {
    const high = jr.jurisdictions.filter(j => j.risk === 'high');
    for (const j of high) {
      const purposeCount = Object.values(j.axes).filter(v => v === 'purpose').length;
      expect(purposeCount).toBeGreaterThanOrEqual(2);
      expect(Object.values(j.axes)).not.toContain('none');
    }
  });
});
...
describe('llm_snapshot.json', () => {
  ...
  it('rootAlgorithms is the documented summary shape', () => {
    expect(snap.rootAlgorithms).toHaveProperty('totalRoots');
    expect(snap.rootAlgorithms).toHaveProperty('byKeyFamily');
    expect(snap.rootAlgorithms).toHaveProperty('byKeyBits');
    expect(snap.rootAlgorithms).toHaveProperty('bySigHash');
    expect(snap.rootAlgorithms).toHaveProperty('perCA');
    expect(Array.isArray(snap.rootAlgorithms.perCA)).toBe(true);
  });
