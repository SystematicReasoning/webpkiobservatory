...
import { COLORS, STORE_COLORS, FONT_MONO, FONT_SANS , ALPHA } from '../constants';
import {
...
const FilterBtn = ({ label, active, color, onClick }) => (
  <button
    ...
    style={{
      ...
      border: `1px solid ${active ? (color || COLORS.ac) : (color ? `color-mix(in srgb, ${color} 27%, transparent)` : COLORS.bd)}`,
      background: active ? `color-mix(in srgb, ${color || COLORS.ac} 20%, transparent)` : 'transparent',
  ...
  return (
    <div>
      ...
      <div style={{ height, display: 'flex', alignItems: 'flex-end', gap: 1 }}>
        {vals.map((v, i) => (
          <div key={keys[i]} title={`${keys[i]}: ${v} bugs`} style={{
  ...
  return (
    <div style={{ position: 'relative', paddingLeft: 18 }}>
      ...
      {merged.map((m, i) => {
        ...
        return (
          <div key={`${a.date || i}-${String(a.summary || '').slice(0,20)}`} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 6, position: 'relative' }}>
  ...
  return (
    <div>
      ...
      <div style={statGridStyle}>
        <StatCard l="Distrust Events" v={stats.total_events || allEvents.length}
          s={allEvents.length > 0 ? `${allEvents.reduce((mn,e)=>Math.min(mn,+(e.year||9999)),9999)}–${allEvents.reduce((mx,e)=>Math.max(mx,+(e.year||0)),0)}` : '2011–2024'} />
        <StatCard
          l="Avg Interval Between Distrusts"
          v={stats.avg_interval_years
            ? `${Math.round(stats.avg_interval_years * 12)} mo`
            : '~11 mo'}
          s="avg time between events"
          c={COLORS.ac}
        />
        <StatCard
          l="Median Time to Distrust"
          v={stats.median_runway_days ? `${(stats.median_runway_days / 365).toFixed(1)} yrs` : '~3.2 yrs'}
          s={`${stats.median_runway_days || 1185} days from first incident`}
          c={COLORS.am}
        />
        <StatCard
          l="Max Time to Distrust"
          v={stats.max_runway_days ? `${(stats.max_runway_days / 365).toFixed(1)} yrs` : '~7.2 yrs'}
          s={`${stats.max_runway_days || 2644} days — longest before action`}
          c={COLORS.rd}
        />
        {(() => {
          ...
          return (
            <StatCard
              ...
              v={`${Math.round(patternCount / total * 100)}%`}
              s={`${patternCount} of ${total} events, spanning multiple years`}
              c={COLORS.rd}
            />
          );
        })()}
        {(() => {
          ...
          return (
            <StatCard
              ...
              v={`${Math.round(opsCount / total * 100)}%`}
              s={`${opsCount} of ${total} events — not just technical errors`}
              c={COLORS.am}
            />
          );
        })()}
        <StatCard l="Response-Quality Failures" v={`${stats.response_driven_pct || 73}%`} s={`${Math.round((stats.response_driven_pct || 73) / 100 * (stats.total_events || allEvents.length))} of ${stats.total_events || allEvents.length} events — minimizing or concealment`} c={COLORS.t2} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 12, marginBottom: 12 }}>
        <Card>
          <CardTitle sub="Compliance posture of each distrusted CA at the time of distrust action — whether the CA had open incidents, pattern-of-issues findings, or was in good standing.">Posture Distribution</CardTitle>
        ...
        <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px 0' }}>
            <CardTitle sub="Incident and failure pattern tags across all distrust events. Tags are LLM-classified from Bugzilla evidence and root program announcements.">Reason Distribution</CardTitle>
