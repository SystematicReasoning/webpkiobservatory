...
const GeoView = () => {
  ...
  return (
    <div>
      <TabIntro tabId="geo" quote="Geography is destiny — especially in PKI.">
      ...
      {(() => {
        ...
        return (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: 16, marginBottom: 16 }}>
            <Card>
              <CardTitle sub="CA organizations by region of headquarters. Click a segment to filter.">
                CAs by Region
              </CardTitle>
              <div
                style={{
            ...
            <Card>
              <CardTitle sub="CA organizations by country of headquarters. Click a segment to see CAs.">
                CAs by Country
              </CardTitle>
              <div style={{ display: 'grid', gridTemplateColumns: selCountry !== null ? 'repeat(auto-fit, minmax(min(100%, 260px), 1fr))' : '1fr', gap: 12 }}>
                <div>
                  ...
                  >
                    ...
                    {countries.length > 8 && (
                      <span style={{ whiteSpace: 'nowrap' }}>+{countries.length - 8} more</span>
                    )}
                  </div>
                </div>
                {selCountry !== null &&
      ...
      <Card>
        <CardTitle sub="Two proportional bars: certificate issuance share (top) and CA organization count (bottom) by region. US dominates issuance far more than it dominates CA count.">
          Issuance vs CA Count by Region
        </CardTitle>
      ...
      {/* Geographic map */}
      <Card>
        <CardTitle sub="Each dot is a country with at least one CA organization. Dot size = certificate issuance volume. Number = CA organization count. Color = geographic region.">
          CA Organizations by Country
        </CardTitle>
        <GeoMap
          height={280}
      ...
      {/* Paired comparison */}
      <Card>
        <CardTitle sub="CA organization count and certificate issuance volume side by side per region. Disproportion between the two signals concentration risk.">
          CA Count and Issuance Volume by Region
        </CardTitle>
        {geography.map((r) => {
          const cl = rc[r.rg] || COLORS.t3;
