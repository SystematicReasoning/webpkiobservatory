...
const RootExpirationHeatmap = ({ roots, caData, heatmapCount, setHeatmapCount }) => {
  ...
  return (
    <Card>
      <CardTitle sub={`${allRootsHM.length} roots across ${caIds.length} of ${trustedCount} trusted CAs. Each cell = roots expiring that year. Years with many simultaneous expirations create coordinated transition pressure — CAs must reissue subscriber certificates before those roots leave trust.`}>
