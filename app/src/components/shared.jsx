 ...
 * Re-renders on resize with 100ms debounce.
 */
...
/**
 * ResponsiveTable — renders a standard <table> on desktop, stacked key-value
 * cards on mobile. Columns and rows are plain arrays so the component can
 * produce both layouts from the same data.
 *
 * Props:
 *   columns: [{ key, label, render?, numeric?, mobileHide? }]
 *     - key: row property name (or used as React key)
 *     - label: header text
 *     - render: optional (value, row) => ReactNode
 *     - numeric: right-align
 *     - mobileHide: omit from mobile card view (for low-priority columns)
 *   rows: array of objects
 *   rowKey: fn(row) => string — unique key per row
 *   mobileTitle: fn(row) => ReactNode — bold first line in mobile card
 *   emptyMessage: string shown when rows is empty
 */
export function ResponsiveTable({ columns, rows, rowKey, mobileTitle, emptyMessage = 'No data' }) {
  const isMobile = useIsMobile();
  const C = COLORS;

  const thStyle = {
    padding: '6px 10px', fontSize: 10, fontWeight: 600, color: C.t3,
    borderBottom: `1px solid ${C.bd}`, textAlign: 'left',
    textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap',
  };
  const tdStyle = {
    padding: '6px 10px', fontSize: 12,
    borderBottom: `1px solid ${C.bd}`, color: C.tx,
  };

  if (!rows.length) {
    return <div style={{ padding: '16px 10px', textAlign: 'center', fontSize: 12, color: C.t3 }}>{emptyMessage}</div>;
  }
        ...
        {rows.map(row => (
          <div key={rowKey(row)} style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 12px' }}>
              {visibleCols.filter(c => !mobileTitle || c.key !== visibleCols[0].key).map(col => {
                const val = col.render ? col.render(row[col.key], row) : row[col.key];
                if (val == null || val === '—' || val === '') return null;
                return (
                  <div key={col.key}>
                    <div style={{ fontSize: 9, color: C.t3, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 1 }}>{col.label}</div>
                    <div style={{ fontSize: 11, color: C.t2 }}>{val}</div>
                  </div>
                );
              })}
            </div>
    ...
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        <thead>
          <tr>
            {columns.map(col => (
              <th key={col.key} style={{ ...thStyle, textAlign: col.numeric ? 'right' : 'left' }}>
                {col.label}
              </th>
            ))}
          </tr>
...
const STORE_NAMES = { mozilla: 'Mozilla', chrome: 'Chrome', microsoft: 'Microsoft', apple: 'Apple' };
...
export const TrustDots = ({ tb, sz = 6 }) => (
  <span style={{ display: 'inline-flex', gap: 2 }}>
    {['mozilla', 'chrome', 'microsoft', 'apple'].map((s) => (
      <span
        key={s}
        title={tb[s] ? STORE_NAMES[s] : `Not in ${STORE_NAMES[s]}`}
        style={{
          width: sz,
          height: sz,
          borderRadius: '50%',
...
export const ChartWrap = ({ height, children, style }) => {
  ...
  return (
    <div ref={ref} style={{ height, width: '100%', minWidth: 0, position: 'relative', overflowX: style?.minWidth ? 'auto' : 'hidden', ...style }}>
      {ready ? (
        children
      ) : (
        <div
...
import { PeculiarCertificateViewer, PeculiarCrlViewer } from '@peculiar/certificates-viewer-react';
...
/** CRL Viewer — wraps @peculiar/certificates-viewer-react PeculiarCrlViewer.
 *  Accepts raw CRL bytes as a Uint8Array or base64 string.
 *  The component handles both DER and PEM formats. */
export const CrlViewer = ({ data }) => {
  if (!data) {
    return (
      <div style={{ padding: '8px 0', fontSize: 9, color: COLORS.t3 }}>
        CRL data not available.
      </div>
    );
  }

  // Convert Uint8Array or raw bytes to base64 string
  let b64;
  if (typeof data === 'string') {
    // Already base64 or PEM — strip PEM headers if present
    b64 = data
      .replace(/-----BEGIN X509 CRL-----/g, '')
      .replace(/-----END X509 CRL-----/g, '')
      .replace(/\s/g, '');
  } else {
    // Uint8Array / Buffer — convert to base64
    b64 = btoa(Array.from(data).map(b => String.fromCharCode(b)).join(''));
  }

  const isDark = document.documentElement.getAttribute('data-theme') !== 'light';
  const theme = isDark ? certViewerDarkTheme : certViewerLightTheme;

  return (
    <div style={{ marginTop: 8, borderRadius: 6, border: `1px solid ${COLORS.bd}`, ...theme }}>
      <PeculiarCrlViewer certificate={b64} download />
    </div>
  );
};

/** Methodology disclosure card — consistent pattern across all tabs */
export const MethodologyCard = ({ children, title }) => (
  <div style={{
    background: COLORS.bg, border: `1px solid ${COLORS.bd}`, borderRadius: 10,
    padding: '14px 16px', marginTop: 16,
  }}>
    <div style={{ fontSize: 14, fontWeight: 600, color: COLORS.tx, marginBottom: 10 }}>
      {title || 'Methodology'}
    </div>
