 ...
 *    dzacharo@reference entity.gr        -> d*******@reference entity.gr  (CA domain — kept)
 *    rob@reference entity.com           -> r**@reference entity.com     (CA domain — kept)
...
const CA_ORG_DOMAINS = new Set([
  ...
  'quovadis.bm', 'quovadis.com',  // acquired by reference entity
  ...
  return (
    <Card>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
        <CardTitle sub="Bugzilla comment activity by the top 5 most active CA organizations per year. Dashed line marks the start of the recent (2021+) window.">
...
const CommunityView = () => {
  const { communityData, incidentsData } = usePipeline();
  const firstBugYear = incidentsData?.years?.[0]?.y || 2014;
  ...
  const [indSort, setIndSort] = useState('score');
  ...
  const indRows = useMemo(() => {
    ...
    const sortFns = {
      ...
      filed:   (a, b) => b.f - a.f,
      ballots: (a, b) => (b.bp * 3 + b.be) - (a.bp * 3 + a.be),
      name:    (a, b) => a.email.localeCompare(b.email),
    };
    return rows.sort(sortFns[indSort] || sortFns.score);
  }, [inds, balInds, isRecent, indSort]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10, gap: 12, flexWrap: 'wrap' }}>
          <CardTitle sub="Bugzilla comments, ballot proposals, and endorsements per CA organization. All CABF CA members shown — zero-engagement members collapsed at the bottom.">
        ...
        <div style={{ ...footnoteStyle, marginTop: 8 }}>
          {isRecent
            ...
            : `All time: ${firstBugYear} to present. `}
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 4 }}>
          <CardTitle sub="Which CA organizations are driving standards evolution. Proposals = authored the ballot language. Endorsements = co-sponsored. Votes excluded — voting is a membership obligation, not a signal of leadership. CAs with zero proposals or endorsements are standards free-riders.">
            Ballot Proposals and Endorsements
          </CardTitle>
          <div style={toggleStyle}>
            {['recent', 'all'].map(v => (
              <button key={v} style={toggleBtn(isRecent ? v === 'recent' : v === 'all')}
                onClick={() => setIsRecent(v === 'recent')}>
                {v === 'recent' ? 'Recent' : 'All Time'}
              </button>
            ))}
          </div>
        </div>
        {balOrgRows.length === 0 ? (
          <DataPending message="Ballot data not yet available — pipeline fetching CA/B Forum ballot records." />
        ) : (
        <div style={{ display: 'flex', gap: 12, marginTop: 12, flexWrap: 'wrap' }}>
          {balOrgRows.map(({ org, p, e, people, wgs }) => (
            <div key={org} style={{
              background: COLORS.s2, borderRadius: 6, padding: '8px 12px',
              border: `1px solid ${COLORS.bd}`, minWidth: 120, flex: '1 1 120px',
            }}>
      ...
      <Card>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 10 }}>
          <thead>
            <tr>
              {[
                { id: 'name',    label: 'Participant',    w: '34%' },
                { id: null,      label: 'CABF',           w: 50    },
                { id: 'bz',      label: 'Bugzilla bugs',  w: null  },
                { id: 'tech',    label: 'Technical',      w: null  },
                { id: 'filed',   label: 'Bugs filed',     w: null  },
                { id: 'ballots', label: 'Ballots',        w: null  },
              ].map(({ id, label, w }) => (
                <th key={label} onClick={id ? () => setIndSort(id) : undefined}
                  style={{
                    ...thStyle,
                    ...(w ? { width: w } : {}),
                    cursor: id ? 'pointer' : 'default',
                    color: indSort === id ? COLORS.ac : COLORS.t3,
                    userSelect: 'none',
                  }}>
                  {label}{indSort === id ? ' ▾' : ''}
                </th>
