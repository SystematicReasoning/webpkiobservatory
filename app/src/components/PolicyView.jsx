...
import { Card, CardTitle, DataPending, StatCard, ChartWrap, TabIntro, MethodologyCard, MethodologyItem } from './shared';
...
const PolicyView = () => {
  ...
  // Require brStatus in bundle — computed by fetch_and_join.py with active-TLS qualification.
  // If missing, the bundle predates this field; show DataPending rather than wrong data.
  const hasBrStatus = data.some(d => d.brStatus);

  // Only include active TLS issuers — brStatus='not_applicable' means legacy cert
  // populations (old S/MIME, code signing, historical cross-signs) that make
  // the usage period meaningless as a BR compliance signal.
  const casWithUsage = useMemo(
    () => data
      .filter((d) => d.brStatus && d.brStatus !== 'not_applicable' && d.avgDays > 0)
      .sort((a, b) => b.avgDays - a.avgDays),
    [data],
  );
  // All CAs with usage data for reference (including legacy/non-TLS)
  const casAllUsage = useMemo(
  ...
  const tiers = [
    ...
    { label: 'Subscribers not ready for 200d', desc: '> 200d avg — subscribers replacing certs too slowly', cas: casWithUsage.filter((c) => c.avgDays > 200), color: COLORS.rd },
  ...
  const excludedLegacy = casAllUsage.length - casWithUsage.length;

  if (!hasBrStatus) {
    return <DataPending tab="Policy Compliance" source="market_share.json" description="BR validity status pipeline (fetch_and_join.py — brStatus field)" />;
  }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 18, fontWeight: 700, color: COLORS.tx, marginBottom: 4 }}>Certificate Validity</div>
      ...
      <div
        style={statGridStyle}
      >
        <StatCard l="Active TLS Issuers" v={casWithUsage.length} s={excludedLegacy > 0 ? `${excludedLegacy} legacy/non-TLS excluded` : undefined} c={COLORS.ac} />
        <StatCard
          l="Subscribers not 200d-ready"
          v={casWithUsage.filter((d) => d.avgDays > 200).length}
          s="avg renewal period exceeds current 200d limit"
          c={COLORS.rd}
        />
            ...
            <table style={compactTableStyle}>
              ...
              <tbody>
                {pagedCAs.map((c) => {
                  ...
                  const th = c.brStatus === 'subscriber_risk' ? '>200d avg'
                    : c.avgDays > 100 ? '100d' : c.avgDays > 47 ? '47d' : '—';
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--t2)', marginTop: 3, maxWidth: 640 }}>
            The regulatory surface CAs must navigate — obligations drawn from CA/B Forum
            requirements, root program policies, IETF standards, audit criteria, and EU
            regulation. Growth in this surface is structural context for the readiness
            picture above.
          </div>
        </div>
      </div>
