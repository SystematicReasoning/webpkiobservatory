...
const TailView = () => {
  const { trustedCAs, browserCoverage } = usePipeline();
  const allStoresCoverage = browserCoverage
    ? Math.round((browserCoverage.chrome + browserCoverage.apple + browserCoverage.mozilla + browserCoverage.microsoft) * 100)
    : 97;
  ...
  return (
    <div>
      ...
      >
        ...
        certificates trusted by ~{allStoresCoverage}% of web browsers whether it issues 500 million certs or 5. Tail CAs in all 4 stores
      ...
      <MethodologyCard>
        ...
        <MethodologyItem label="Store grouping">{`Tail CAs grouped by trust store presence (4, 3, 2, or 1 store). Tail CAs in all 4 stores represent the highest risk-to-utility ratio: maximum blast radius (~${allStoresCoverage}% web coverage) with minimal ecosystem contribution.`}</MethodologyItem>
