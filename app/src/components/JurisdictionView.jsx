...
import { COLORS, FONT_MONO, FONT_SANS, COUNTRY_COORDS , ALPHA } from '../constants';
...
const JurisdictionView = () => {
  ...
  // reference entity, reference entity are excluded — they are no longer in any trust store and
    ...
    return laws
      .map((j) => {
        ...
        return {
          ...
          count: exp?.count > 0 ? exp.count : null,
          tooltip: (
            <div>
  ...
  return (
    <div>
      <TabIntro tabId="jurisdiction" quote="Trust is global. Law and risk are local.">
      ...
      {/* ── Map ── */}
      <Card>
        <CardTitle sub="Pin color = compulsion risk level. Pin size = issuance volume. Number inside pin = CA organization count.">
          CA Exposure by Jurisdiction Risk
        </CardTitle>
        <GeoMap
          pins={pins}
          legend={[
