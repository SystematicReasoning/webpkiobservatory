      ...
      <table style={compactTableStyle}>
        <thead style={{ position: 'sticky', top: 0, background: COLORS.s1 }}>
          <tr style={{ borderBottom: `1px solid ${COLORS.bd}` }}>
            {[['CA', 'CA organization name'], ['Type', 'Government-operated or state-owned enterprise'], ['Jurisdiction', 'Country of government affiliation'], ['Compulsion', 'Jurisdiction compulsion risk level from jurisdiction_risk.json — whether laws exist permitting government to compel key disclosure, certificate issuance, or impose secrecy on auditors. High/moderate/low based on cited statutes only; — means no entry yet.'], ['Relationship', 'Nature of government structural tie'], ['Stores', 'Trust store inclusion count (out of 4)'], ['Certs', 'Unexpired precertificates'], ['Ops‡', 'Incident rate per million certificates']].map(([h, tip], i) => (
              <th
                key={h}
                title={tip}
                style={{
                  ...
                  textAlign: [5, 6].includes(i) ? 'right' : i === 7 ? 'center' : 'left',
                  cursor: 'help',
                }}
              >
                {h}
                  ...
                  <td style={{ padding: '4px 5px', textAlign: 'center' }}>
                    {c.jurisdictionRisk ? (
                      <span style={{
                        ...
                        borderRadius: 3,
                        color: '#fff',
                        background: c.jurisdictionRisk === 'high' ? COLORS.rd
                          : c.jurisdictionRisk === 'moderate' ? COLORS.am
                          : COLORS.gn,
                      }}>
                        {c.jurisdictionRisk}
                      </span>
                    ) : (
                      <span style={{ fontSize: 9, color: COLORS.t3 }}>—</span>
                    )}
                  </td>
                  <td
                    title={c.influence}
...
const GovView = () => {
  ...
  return (
    <div>
      <TabIntro tabId="gov" quote="When the state is the CA, the threat model changes.">
      ...
      <Card>
        <CardTitle sub="Government-operated and state-owned-enterprise CAs plotted by headquarters country. Dot size scales with number of CAs. Color distinguishes ownership type.">
          Government CAs by Jurisdiction
        </CardTitle>
        <GeoMap
          pins={(() => {
            // Group CAs by jurisdiction first, then build pins
            const byCountry = {};
            govRisk.cas.forEach((c) => {
              const co = COUNTRY_COORDS[c.jurisdiction];
              if (!co) return;
              if (!byCountry[c.jurisdiction]) {
                byCountry[c.jurisdiction] = { co, cas: [], types: new Set() };
              }
              byCountry[c.jurisdiction].cas.push(c);
              byCountry[c.jurisdiction].types.add(c.type);
            });
            return Object.entries(byCountry).map(([country, { co, cas, types }]) => {
              const n = cas.length;
              // Mixed: use amber; all SOE: use cyan; all GO: use amber
              const color = types.has('GO') ? COLORS.am : COLORS.cy;
              return {
                lat: co.lat,
                lng: co.lng,
                label: country,
                color,
                r: Math.max(5, Math.min(14, 4 + Math.sqrt(n) * 3)),
                count: n,
                tooltip: (
                  <div>
                    <div style={{ fontWeight: 600, color: COLORS.tx }}>{country}</div>
                    <div style={{ color: COLORS.t2 }}>{n} CA{n > 1 ? 's' : ''}</div>
                    <div style={{ color: COLORS.t3, fontSize: 9 }}>{cas.map((c) => c.caOwner).join(' · ')}</div>
                  </div>
                ),
              };
            });
      ...
      <MethodologyCard>
        ...
        <MethodologyItem label="Compulsion Risk">
          The Compulsion Risk column joins each CA to jurisdiction_risk.json using the CA&apos;s country of incorporation.
          Risk levels are based on cited statutes only — not subjective assessments. Three axes are assessed per jurisdiction:
          key seizure (can the government compel disclosure of CA signing keys?), compelled issuance (can it force issuance of a specific certificate?),
          and secrecy (can it prohibit the CA from disclosing the compulsion to auditors or root programs?).
          Each axis is rated purpose-built statute, general judicial process, or none. The overall risk level follows from the combination.
          Jurisdictions without a citable law are left blank rather than rated.
        </MethodologyItem>
