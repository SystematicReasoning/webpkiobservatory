...
if (inc) {
  ...
  if (inc.yearsByClass && inc.yearsByClass.length > 0) {
    ...
    const unclassified = inc.meta?.unclassified_count || 0;
    const gap = inc.total - classSum;
    // Allow gap up to the reported unclassified count (some bugs have no LLM category)
    check('yearsByClass matches total', gap >= 0 && gap <= Math.max(unclassified + 5, 10),
      `classified=${classSum} total=${inc.total} gap=${gap} unclassified=${unclassified}`);
  }
}
