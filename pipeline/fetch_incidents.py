...
from utils import load_json, save_json, ANTHROPIC_URL, ANTHROPIC_MODEL, ANTHROPIC_VERSION
from config import CA_SELF_REPORT_DOMAINS

# Force unbuffered stdout so CI shows progress in real time
# Force unbuffered stdout for CI — only when run directly, not when imported by tests
...
# Bump this when the classification prompt changes to force re-classification of all bugs
CLASSIFICATION_PROMPT_VERSION = "v2"

# CA email domains — used only as a fallback signal for the LLM prompt context,
# and for bugs that haven't been classified yet
# CA email domain list moved to config.py as CA_SELF_REPORT_DOMAINS
CA_EMAILS = CA_SELF_REPORT_DOMAINS
...
- "self": true if the CA itself discovered and reported the issue, false if discovered by an external party (root program, researcher, relying party, browser vendor, or automated scan)
...
For "self": In Mozilla Bugzilla CA compliance bugs, self-reported means the CA proactively filed the bug themselves before being contacted. Signals of self-reporting include:
- The bug summary or description says the CA "self-reported", "discovered internally", "proactively discloses", or "identified by" the CA
- The bug title starts with the CA name followed by a colon, describing an issue they are disclosing
Signals of external discovery include:
- The summary says "identified by" a browser vendor, root program, or researcher
- The bug describes a complaint or finding from an external party
...
def is_self_reported_email(creator):
    """Check if a bug creator email domain matches a known CA domain.

    This is the domain-based attribution method, used when LLM classification
    has not yet been cached for a bug. When the LLM cache is fully populated
    (normal operation), this function is not called — the LLM 'self' field
    takes precedence. When new bugs arrive before the next LLM classification
    run, this provides a best-effort attribution based on the creator's domain.
    """
    if not creator:
        return False
...
def classify_bugs(bugs, existing_classifications, api_key):
    ...
    # Find bugs needing classification or re-classification (prompt version changed)
    needs = []
    for b in bugs:
        bug_id = str(b["id"])
        existing = existing_classifications.get(bug_id)
        if (not existing
                or existing.get("summary") != b["summary"]
                or existing.get("prompt_version") != CLASSIFICATION_PROMPT_VERSION):
            needs.append(b)
    ...
    for i in range(0, len(needs), CLASSIFICATION_BATCH_SIZE):
        ...
        summaries = "\n".join(f"Bug {b['id']} (filed by {b.get('creator','unknown')}): {b['summary']}" for b in batch)
...
def build_incidents_json(all_bugs, mappings, classifications, meta):
    ...
    Schema:
    {
        ...
        "fingerprints": [{"ca": "reference entity", "mi": 66, "rv": 25, "gv": 19, "vl": 15}, ...],
        "cas": [{"ca": "reference entity", "n": 171, "self": 87, "ext": 84, "selfPct": 51}, ...],
        "distrusted_excluded": [{"ca": "reference entity", "n": 0}, ...]
    }
    ...
    bugs_by_ca = defaultdict(list)    # ca_owner -> [{id, filed, summary, self_reported}]
    bugs_by_ca_distrusted = defaultdict(list)  # same schema, for distrusted CAs only
    ...
    for b in all_bugs:
        ...
        if entry["status"] == "distrusted":
            ...
            # Preserve full bug detail for distrust retrospective in fetch_audits.py.
            # ccadb_owner is None for distrusted entries — use the raw CA name as key
            # so bugs are grouped correctly. fetch_audits.py maps these via the
            # distrust events file (ca_owner field) to match audit profiles.
            dist_key = entry.get("ccadb_owner") or raw_ca
            cls_d = classifications.get(str(b["id"]))
            sr_d = cls_d["self"] if cls_d and "self" in cls_d else is_self_reported_email(b.get("creator", ""))
            bugs_by_ca_distrusted[dist_key].append({
                "id":            b["id"],
                "filed":         b["creation_time"][:10],
                "summary":       b["summary"],
                "self_reported": sr_d,
                "whiteboard":    b.get("whiteboard", ""),
            })
            continue

        canonical = entry["ccadb_owner"]
        cls_early = classifications.get(str(b["id"]))
        if cls_early and "self" in cls_early:
            sr = cls_early["self"]
        else:
            sr = is_self_reported_email(b.get("creator", ""))

        # Track per-bug detail for retrospective analysis
        bugs_by_ca[canonical].append({
            "id":            b["id"],
            "filed":         b["creation_time"][:10],  # YYYY-MM-DD
            "summary":       b["summary"],
            "self_reported": sr,
            "whiteboard":    b.get("whiteboard", ""),
        })
    ...
    return output, unmapped, dict(bugs_by_ca), dict(bugs_by_ca_distrusted)
...
def main():
    ...
    classifications = load_json(PIPELINE_DIR / "incident_classifications.json", {})
    ...
    if has_new or any(str(b["id"]) not in classifications for b in cached_bugs[:10]):
        ...
        save_json(PIPELINE_DIR / "incident_classifications.json", classifications)
    ...
    # Step 3: Build output
    print("\n--- Build Output ---")
    output, unmapped, bugs_by_ca, bugs_by_ca_distrusted = build_incidents_json(
        cached_bugs, mappings, classifications, meta
    )

    save_json(OUTPUT_DIR / "incidents.json", output)
    save_json(OUTPUT_DIR / "bugs_by_ca.json", bugs_by_ca)
    save_json(OUTPUT_DIR / "bugs_by_ca_distrusted.json", bugs_by_ca_distrusted)

    total_size = os.path.getsize(OUTPUT_DIR / "incidents.json")
    bca_size   = os.path.getsize(OUTPUT_DIR / "bugs_by_ca.json")
    dist_size  = os.path.getsize(OUTPUT_DIR / "bugs_by_ca_distrusted.json")
    dist_bugs  = sum(len(v) for v in bugs_by_ca_distrusted.values())
    print(f"  Wrote incidents.json ({total_size / 1024:.1f} KB)")
    print(f"  Wrote bugs_by_ca.json ({bca_size / 1024:.1f} KB — {sum(len(v) for v in bugs_by_ca.values())} bugs across {len(bugs_by_ca)} CAs)")
    print(f"  Wrote bugs_by_ca_distrusted.json ({dist_size / 1024:.1f} KB — {dist_bugs} bugs across {len(bugs_by_ca_distrusted)} distrusted CAs)")
