...
def compute_changelog():
    ...
    for i in range(1, len(snapshots)):
        prev = json.loads(Path(snapshots[i - 1]).read_text(encoding="utf-8"))
        curr = json.loads(Path(snapshots[i]).read_text(encoding="utf-8"))
        prev_date = prev["date"]
        curr_date = curr["date"]
...
def main():
    ...
    if snap_path.exists():
        print(f"  Snapshot for {today} already exists — skipping fetch and changelog recompute")
        return
    ...
    # Compute changelog from all snapshots (only runs when a new snapshot was taken)
    compute_changelog()
