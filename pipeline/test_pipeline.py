...
def test_slugify():
    ...
    assert slugify('Example Security Research Group') == 'example-security-research-group'
    assert slugify('Example Trust GmbH') == 'example-trust-gmbh'
    assert slugify('') == ''
    assert slugify(None) == ''
...
def test_build_incidents_json_basic():
    ...
    bugs = [
        _make_bug(1001, 'Fixture Alpha', year='2023'),
        _make_bug(1002, 'Fixture Alpha', year='2024'),
        _make_bug(1003, 'Fixture Beta', year='2024'),
    ]
    mappings = {
        'Fixture Alpha': _make_mapping('Fixture Alpha', 'Fixture Alpha'),
        'Fixture Beta': _make_mapping('Fixture Beta', 'Fixture Beta'),
    }
    classifications = {}

    result, unmapped, _bca, _bca_d = build_incidents_json(bugs, mappings, classifications, _make_meta())
...
def test_build_incidents_json_empty():
    from fetch_incidents import build_incidents_json
    result, unmapped, _bca, _bca_d = build_incidents_json([], {}, {}, _make_meta())
...
def test_build_incidents_json_unmapped():
    ...
    result, unmapped, _bca, _bca_d = build_incidents_json(bugs, {}, {}, _make_meta())
    assert result['total'] == 0
...
def test_build_incidents_json_distrusted_excluded():
    from fetch_incidents import build_incidents_json
    bugs = [_make_bug(1001, 'Fixture Gamma')]
    mappings = {'Fixture Gamma': {'ccadb_owner': 'Fixture Gamma', 'status': 'distrusted'}}
    result, _unmapped, _bca, _bca_d = build_incidents_json(bugs, mappings, {}, _make_meta())
...
def test_build_incidents_whiteboard_tags():
    from fetch_incidents import build_incidents_json
    bugs = [_make_bug(1001, 'Fixture Alpha')]
    mappings = {'Fixture Alpha': _make_mapping('Fixture Alpha', 'Fixture Alpha')}
    result, _unmapped, _bca, _bca_d = build_incidents_json(bugs, mappings, {}, _make_meta())
...
def test_build_incidents_year_grouping():
    from fetch_incidents import build_incidents_json
    bugs = [
        _make_bug(1001, 'Fixture Alpha', year='2022'),
        _make_bug(1002, 'Fixture Alpha', year='2022'),
        _make_bug(1003, 'Fixture Alpha', year='2024'),
    ]
    mappings = {'Fixture Alpha': _make_mapping('Fixture Alpha', 'Fixture Alpha')}
    result, _unmapped, _bca, _bca_d = build_incidents_json(bugs, mappings, {}, _make_meta())
...
def _make_minimal_snapshot():
    ...
    return {
        ...
        'ecosystemParticipation': {
            ...
            'topOrganizations': [{'name': 'Fixture Beta', 'bugzillaEngaged': 100}],
            'topBallotIndividuals': [{'name': 'Participant Example', 'proposed': 38, 'endorsed': 1}],
        },
...
def test_roundtrip_incidents_output():
    ...
    bugs = [_make_bug(i, 'Fixture Alpha') for i in range(10)]
    mappings = {'Fixture Alpha': _make_mapping('Fixture Alpha', 'Fixture Alpha')}
    result, _unmapped, _bca, _bca_d = build_incidents_json(bugs, mappings, {}, _make_meta())
