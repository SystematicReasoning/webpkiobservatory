...
def fetch_statcounter():
    """Fetch browser market share from the StatCounter HTML page."""
...
def main():
    ...
    browser_data = fetch_statcounter()

    using_fallback = False
    if not browser_data:
        # StatCounter fetch failed. Use hardcoded values rather than failing the whole
        # pipeline run — a StatCounter outage should not block crt.sh/CCADB/Bugzilla
        # data from updating. The fallback is flagged in the output so the dashboard
        # footer can show a stale-data warning, and CI logs make it visible.
        print("  WARN: Could not fetch live data from StatCounter — using hardcoded fallback")
        print("  WARN: Web coverage percentages may be out of date")
        using_fallback = True
        browser_data = {
            "Chrome": 68.98,
            "Safari": 16.39,
