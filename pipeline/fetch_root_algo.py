...
in ~6 seconds. Roots not found in the CCADB bulk download are logged as
warnings but not fetched from external sources.

Also saves pipeline/pem_cache.json with all PEMs for embedding in per-CA
detail files by fetch_and_join.py (used by the Peculiar certificate viewer).
...
def fetch_bulk_pems():
    ...
    # Derive decade range from current year so 2030+ roots are not silently missed.
    start_decade = 1990
    end_decade = (datetime.now().year // 10) * 10
    decades = [str(d) for d in range(start_decade, end_decade + 10, 10)]
...
def parse_cert(pem_text, fingerprint="unknown"):
    ...
    sig_oid = cert.signature_algorithm_oid.dotted_string if cert.signature_algorithm_oid else "unknown"
...
def build_output(roots_with_algo):
    ...
    for r in roots_with_algo:
        algo = r.get("algo", {})
        if "error" in algo:
            skipped_errors.append(f"{r['root_name'][:50]} ({algo['error']})")
            continue
        records.append({
            "ca_id": r["ca_slug"],
            "ca_owner": r["ca_owner"],
    ...
    if skipped_errors:
        print(f"  WARNING: {len(skipped_errors)} roots excluded from output (parse/fetch errors):",
...
def main():
    ...
    # Always fetch: even if algo cache is complete, we need PEMs for the cert viewer
    bulk_pems = fetch_bulk_pems()

    if uncached:
        # Step 4: Match and parse uncached roots
        matched_bulk = 0
        missed = []
        ...
        if missed:
            print(f"  WARNING: {len(missed)} roots not found in CCADB bulk PEM download:")
            for root in missed:
                print(f"    {root['root_name'][:60]} ({root['sha256'][:16]}...)")
                cache[root["sha256"]] = {"error": "not_in_ccadb", "fingerprint": root["sha256"]}
