...
# Deduplicate CA names that differ only in capitalisation
# e.g. 'e-commerce monitoring gmbh' and 'e-commerce monitoring GmbH'
# Keep the canonical (first-seen) entry, discard the variant
...
def _market_inverse(ca_name: str) -> float:
    """
    Small CA premium: a tiny CA with poor compliance posture warrants more
    urgent engagement than a large CA with similar raw signals — the root
    program has less leverage and the CA has less incentive to remediate.
    This is purely operational/economic, not geopolitical.
    Returns 0.0 (large) to 0.8 (tail-end).
    """
    pct = _market_share(ca_name) or 0.0
    if pct >= 5.0:  return 0.0
    if pct >= 1.0:  return 0.2
...
def compute_eps(ca_name: str, als_total: float, irq_scores: dict,
    ...
    """
    Multi-dimensional Engagement Priority Score (0-100).

    Three components — all purely behavioral and structural:
      55% Behavioral quality per incident
          - pc_ignored_rate, obligation/candor failure rates, RCA depth,
            chronic density, acceleration (existing signals)
          - severity_score: incident type weighting — ca-misissuance (sev5)
            outweighs 16 attribute misissuances. Ryan: "A missed CA cert is
            much more indicative than boinking a few dozen OV/EV attributes."
          - recency_score: last 12 months severity-weighted — a CA on a recent
            bad run scores higher. Ryan: "Microsoft, no CA with worse performance
            in Bugzilla over the last 9 months."
          - recent_ap_rate: fraction of recent bugs requiring community escalation.
            Ryan: "quality of responses to questions from the community."
      20% ALS structural accumulation — chronic class depth still matters.
      25% Market inverse — small CA premium: lower accountability pressure,
          lower remediation incentive, lower cost of distrust. Purely
          operational — not a geopolitical or ownership judgment.
    """
    if population != "active":
        return round(min(als_total * 0.7 + (irq_scores.get("irq_failure", 0) or 0), 100), 1)
    ...
    density      = sig.get("n_chronic", 0) / max(n, 1)
    sev_score    = sig.get("severity_score",  0.0)
    recency      = sig.get("recency_score",   0.0)
    recent_ap    = sig.get("recent_ap_rate",  0.0)
    # Fraction of bugs the CA did NOT self-report (externally detected).
    # Sectigo finds 61% of its own problems; DigiCert only 47%. A CA that
    # relies on external detection for 53% of incidents has a detection problem.
    external_detection = 1.0 - (sig.get("self_pct", 50) / 100.0)
    # Has this CA ever issued a malformed or non-compliant CA certificate?
    # This is categorically different from attribute misissuance — it represents
    # a failure of the PKI hierarchy itself. Saturates at 3 incidents (max=1.0).
    sev5_depth = min(sig.get("n_sev5", 0) / 3.0, 1.0)

    # Recency: severity-weighted score for last 12 months, scaled by volume
    # and accountability pressure rate. Primary signal per Ryan Dickson —
    # what has the CA done recently, and how have they responded?
    recency_vol = min(sig.get("n_recent", 0) / 12.0, 1.0)
    recency_composite = recency * (0.5 + 0.5 * recency_vol) * (1.0 + min(recent_ap / 0.5, 1.0) * 0.5)

    behavioral = (
        min(pc_rate / 0.3, 1.0)                               * 0.08 +
        oblig                                                  * 0.05 +
        cand                                                   * 0.04 +
        max(0, (3 - rca) / 2)                                  * 0.04 +
        min(density / 0.15, 1.0)                               * 0.07 +
        sev_score                                              * 0.12 +  # alltime severity depth
        sev5_depth                                             * 0.15 +  # CA-level incident history
        external_detection                                     * 0.10 +  # detection failure rate
        recency_composite                                      * 0.35    # recent run is primary
    )

    inv_w = _market_inverse(ca_name)

    raw = behavioral * 0.60 + (als_total / 80) * 0.15 + inv_w * 0.25
