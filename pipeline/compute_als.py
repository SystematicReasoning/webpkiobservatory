...
# Incident severity hierarchy — Ryan Dickson:
# "A missed CA certificate is much more indicative than boinking a few dozen
# OV/EV attributes." Tier reflects blast radius and irreversibility.
...
def analyze_bug_record(bugs: list[dict],
                       ...
                       irq_cache: dict | None = None) -> dict | None:
    """
    ...
    # Captures Ryan's "last 9 months" observation — a CA on a recent bad run
    # should score higher than one with the same all-time average but improving.
    ...
    # Fraction of recent (12mo) bugs where the community had to escalate to get
    # a response — proxy for "quality of CA's responses to community questions."
    _cache = irq_cache or {}
    recent_ap_count = sum(
...
def compute_als(sig: dict, audit_profile: dict | None = None) -> dict:
    ...
    Mode A — Operational blindness + deterioration (Certinomis profile):
      Fires when a CA cannot find its own problems AND the rate is worsening.
      Signals: detection failure (confidence-weighted), acceleration, gov share.

    Mode B — Structural accumulation (Camerfirma/Entrust profile):
      Fires when chronic classes have built up at high density over many years.
      Signals: n_chronic, year_density, chronic_span_avg, gov share.

    Acceleration boost: amplifies both modes when accel > 1x.

    Calibration (from distrusted CAs as test vectors):
      min_distrust = 55.8 (Certinomis)
      max_healthy  = 41.0 (ISRG, Telia)
      gap          = 14.9 pts
      threshold    = 48 (7pt buffer above healthy ceiling)

    Test vector result: 100% sensitivity, 100% specificity.
    Flagged active CAs: 15/52 (29%).
    ...
    # Effective chronic count: subtract closed loops (loops that genuinely closed).
    n_ch_eff    = max(0, n_ch - n_closed)
    # Broken promise boost: a modest amplifier for explicitly broken commitments.
    # Kept deliberately small (max 1.3) — the knowledge weight already captures
    # most of the "should have known better" signal. Broken promise adds the
    # specific "said they fixed it and didn't" dimension.
    broken_boost = min(1.0 + 0.1 * n_broken, 1.3) if n_broken > 0 else 1.0
...
def main():
    ...
    # ── Behavior-adjusted threshold helper ─────────────────────────────────
    # A CA that consistently ignores pattern connections, fails obligation
    # understanding, or produces poor RCA is sending a clear signal even with
    # a modest Bugzilla record. Reduce the flagging threshold proportionally.
    # Max reduction: 14 pts (floor: 34). Applies to active CAs only.
    def behavior_tau(irq_scores: dict, sig: dict) -> int:
        base     = 48
        n        = sig.get('n', 1)
        accel    = sig.get('accel', 1.0)
        chronic  = set(sig.get('chronic_classes', {}).keys())
        pc_rate  = (irq_scores.get('pattern_connections_ignored', 0) or 0) / max(n, 1)
        oblig    = irq_scores.get('oblig_failure_rate',  0) or 0
        cand     = irq_scores.get('candor_failure_rate', 0) or 0
        rca      = irq_scores.get('avg_rca_depth',       2) or 2
        reduction = 0
        # Pattern connection ignore rate — clearest incompetence signal
        if   pc_rate >= 0.25: reduction += 4
        elif pc_rate >= 0.15: reduction += 2
        # Obligation failure — CA doesn't understand the requirements it violated
        if   oblig >= 0.25: reduction += 3
        elif oblig >= 0.15: reduction += 1
        # Candor failure — incomplete or evasive disclosure
        if   cand >= 0.30: reduction += 2
        elif cand >= 0.20: reduction += 1
        # RCA depth — proximate-only analysis despite multiple incidents
        if   rca <= 1.8: reduction += 2
        elif rca <= 2.0: reduction += 1
        # disclosure-failure as a chronic class is direct evidence of
        # non-compliance with incident reporting requirements
        if 'disclosure-failure' in chronic: reduction += 3
        # High acceleration with thin record — trend is clear even if total is low
        if accel >= 3.0 and n >= 6: reduction += 2
        elif accel >= 2.0 and n >= 8: reduction += 1
        return max(base - reduction, 34)
    ...
    for ca, bugs in bugs_active.items():
        ...
        # Apply behavior-adjusted threshold using IRQ signals
        irq_s = irq_ca_scores.get(ca, {})
        tau   = behavior_tau(irq_s, sig) if irq_s else 48
    ...
    output = {
        ...
        'test_vector_evaluation': {
            ...
            'note': (
                'False positives (Symantec, e-Tugra) are argued to be correct '
                'structural identifications — both had genuine non-remediation '
                'patterns; their pathways differed due to external factors. '
