...
from utils import parse_ccadb_date, now_utc as _now_utc
...
def fetch_crtsh_populations():
    ...
    # variant spellings like "reference entity" / "reference entity" / "reference entity GmbH"
...
def build_trust_store_profile(ccadb_records):
    ...
    for ca_owner, data in owners.items():
        ...
        for root in data["roots"]:
            ...
            if not is_included:
                continue
            # Check expiration. CCADB uses '%Y.%m.%d' consistently.
            # parse_ccadb_date returns None for absent/unparseable dates.
            # Conservative default: None -> expired=True (exclude).
            # If CCADB ever introduces a new date format we exclude the root
            # rather than silently include a cert whose expiry is unknown.
            # Verified: all 688 current roots parse correctly; this path is unreachable.
            vt_raw = root.get("valid_to", "")
            exp = parse_ccadb_date(vt_raw)
            if exp is None and vt_raw:
                import sys as _sys
                print(f"  WARNING: unparseable valid_to {vt_raw!r} for root "
                      f"{root.get('sha256','')[:16]} — treating as expired",
                      file=_sys.stderr)
            expired = exp is None or exp < now_utc
            if not expired:
        ...
        for inter in data["intermediates"]:
            if inter.get("parent_sha256") not in included_root_fps:
                continue
            # Check intermediate expiration. Same CCADB '%Y.%m.%d' format.
            # Conservative default: None -> expired (exclude).
            vt_raw = inter.get("valid_to", "")
            exp = parse_ccadb_date(vt_raw)
            if exp is None and vt_raw:
                import sys as _sys
                print(f"  WARNING: unparseable valid_to {vt_raw!r} for intermediate "
                      f"{inter.get('sha256','')[:16]} — treating as expired",
                      file=_sys.stderr)
...
def join_data(crtsh_data, ccadb_profiles):
    ...
    # Add BR validity status — only meaningful for active TLS issuers.
    # Uses unexpired_precerts (CT log data) to match the avgDays computation
    # in export_ui_bundle.py which also uses all_precerts / unexpired_precerts.
    # A CA with large legacy non-TLS cert populations (S/MIME, code signing,
    # or historical cross-signs) will show inflated usage periods. Filter:
    #   - tls_capable = True
    #   - unexpired_precerts >= 1000 (genuine active TLS issuance from CT logs)
    #   - trusted by at least one current browser store
    # CAs that don't meet these criteria get br_status = "not_applicable".
    ACTIVE_TLS_MIN_CERTS = 1000
    for rec in joined:
        all_time = rec.get("all_precerts") or 0
        unexpired = rec.get("unexpired_precerts") or 0   # use precerts — matches avgDays
        tls = rec.get("tls_capable", False)
        trusted = any(rec.get("trusted_by", {}).values())

        if not tls or unexpired < ACTIVE_TLS_MIN_CERTS or not trusted or not all_time:
            rec["br_status"] = "not_applicable"
        else:
            use_days = round(365 / (all_time / unexpired))
            rec["use_days"] = use_days
            if use_days > 200:
                rec["br_status"] = "subscriber_risk"
            elif use_days > 100:
                rec["br_status"] = "risk_2027"
            elif use_days > 47:
                rec["br_status"] = "risk_2029"
            else:
                rec["br_status"] = "compliant"
...
def detect_cross_sign_attribution(ccadb_records):
    ...
    for r in ccadb_records:
        ...
        # Check if expired. CCADB uses '%Y.%m.%d' consistently.
        # Conservative default: None -> expired=True (exclude from active set).
        exp = parse_ccadb_date(r.get("Valid To (GMT)", ""))
...
def main():
    ...
    if crtsh_available:
        ...
        print("  WARNING: crt.sh unavailable — using previous market data for certificate counts")
        print("  WARNING: market share, concentration, and BR compliance metrics reflect yesterday's data")
