...
def main():
    ...
    cache = json.loads(cache_path.read_text(encoding="utf-8")) if cache_path.exists() else {"commits": {}}
    ...
    try:
        ...
        print(f"  Failed to fetch commit log: {e}")
        print("  Using cached data if available")
        if cache.get("cached_changelog"):
            print(f"  Cache has {len(cache['commits'])} commits — writing from cache")
            out_path = OUTPUT_DIR / "chrome_root_store_changelog.json"
            with open(out_path, "w", encoding="utf-8") as f:
                json.dump(cache["cached_changelog"], f, indent=2)
        else:
    ...
    # Early-exit: if the latest commit is already in cache and matches stored head, no new data
    latest_commit = commits[0]["commit"] if commits else None
    cached_head = cache.get("head_commit")
    if latest_commit and latest_commit == cached_head and cache.get("cached_changelog"):
        print(f"  Latest commit {latest_commit[:12]} already processed — skipping")
        out_path = OUTPUT_DIR / "chrome_root_store_changelog.json"
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(cache["cached_changelog"], f, indent=2)
        return
    ...
    # Save cache — include head commit hash and full output for early-exit on next run
    cache["head_commit"] = latest_commit
    cache["cached_changelog"] = output
