...
def main():
    ...
    # Build lookup: country -> {risk, axes} for joining into gov CA records
    jur_map = {
        j["country"]: {"risk": j["risk"], "axes": j.get("axes", {})}
    ...
    compliance_growth = load_json(DATA_DIR, "compliance_growth.json")
    audits_data = load_json(DATA_DIR, "audits.json")
    crl_health_raw  = load_json(DATA_DIR, "crl_health.json")

    # CRL URL list is too large to bake into the Vite bundle (9856+ URLs ~35MB).
    # Bundle only the summary and a small sample of issue URLs for the cards.
    # The full URL list is served separately as crl_health.json fetched at runtime.
    if crl_health_raw and "urls" in crl_health_raw:
        issue_urls = [u for u in crl_health_raw["urls"] if u.get("status") != "ok"][:500]
        crl_health_data = {**crl_health_raw, "urls": issue_urls}
    else:
        crl_health_data = crl_health_raw
    crl_health_history = load_json(DATA_DIR, "crl_health_history.json")
    crl_health_events = load_json(DATA_DIR, "crl_health_events.json")
    ...
    for ca in market_share:
        ...
        entry = {
            ...
            # BR validity readiness — qualified: only meaningful for active TLS issuers
            # (tls_capable=True, unexpired>=1000, trusted by at least one store).
            # "not_applicable" = legacy certs, non-TLS, or no current store trust.
            "brStatus": ca.get("br_status", "not_applicable"),
        }
        D.append(entry)
    ...
    for ca in gov_risk.get("classified_cas", []):
        ...
        if ca_owner in DISTRUST_OVERRIDES:
            continue
        # Only include government-operated and state-owned enterprise types.
        # classified_cas contains all classified CAs (including commercial) for
        # the full detail table, but the summary counts should only reflect
        # CAs with a structural government relationship.
        ca_type = ca.get("type", "")
        if ca_type not in type_code_map:
            continue
        ca_jurisdiction = norm_country(ca.get("jurisdiction", ""))
        jur_data = jur_map.get(ca_jurisdiction, {})
        trusted_gov_cas.append({
            "caOwner": ca_owner,
            "type": type_code_map[ca_type],
            ...
            "jurisdictionRisk": jur_data.get("risk"),
            "jurisdictionAxes": jur_data.get("axes"),
        })
    ...
    output = {
        ...
        "COMPLIANCE_DATA": compliance_growth,
        "AUDITS_DATA": audits_data,
        "CRL_HEALTH_DATA": crl_health_data,
        "CRL_HEALTH_HISTORY": crl_health_history,
        "CRL_HEALTH_EVENTS": crl_health_events,
    }
    ...
    # Also write distrust_data.json as a standalone file for consistency
    # with the rest of the data/ directory (even though the source is
    # hand-curated in pipeline/distrust/distrusted.json)
    distrust_path = os.path.join(DATA_DIR, "distrust_data.json")
    with open(distrust_path, "w", encoding="utf-8") as f:
        json.dump(distrust_data, f, indent=2, ensure_ascii=False)
    ...
    if audits_data:
        parsed_count = audits_data.get("summary", {}).get("pdf_parsed_count", 0)
        profiles_count = len(audits_data.get("profiles", []))
        print(f"  AUDITS_DATA: {profiles_count} profiles, {parsed_count} parsed letters")
    else:
        print(f"  AUDITS_DATA: missing — audits.json not found or empty")
