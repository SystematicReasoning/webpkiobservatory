...
from utils import load_json, ANTHROPIC_URL, ANTHROPIC_MODEL, ANTHROPIC_VERSION

# Force unbuffered stdout
if __name__ == "__main__" and not sys.stdout.isatty():
...
- "technical": true if the comment contains technical findings or analysis, false if it is process enforcement only
  (technical is only meaningful when governance is true; set false whenever governance is false)
...
- Asking pointed questions about the scope, root cause, or timeline of an incident
- Noting that a report is overdue or incomplete and demanding a full response
...
- Requiring a CA to file a follow-up bug as a pure process step with no technical content
- Test bug filings or browser bug report templates
- Comments that are purely procedural with no technical content

When in doubt on governance, lean toward true.

TECHNICAL classification (only applies when governance is true):

Technical finding (true) — the comment contains at least one of:
- Direct certificate or CRL analysis (field values, serial numbers, hash values, encoding inspection)
- A crt.sh, censys, or CT log link citing a specific misissued certificate
- A specific BR/RFC/policy section cited with evidence of violation, not just a process requirement
- Quantification of scope (X certificates affected, Y% revoked, Z days elapsed with calculation)
- Identification of a root cause the CA had not stated
- A specific technical question requiring the CA to provide certificate data or logs

Process enforcement only (false) — the comment is governance but purely procedural:
- Demanding a response or follow-up report without technical content
- Noting a deadline was missed without analyzing the underlying incident
- Requesting the CA file a separate bug as a housekeeping step
- Asking the CA to confirm a process step (audit filed, survey submitted, etc.)

--- FEW-SHOT EXAMPLES ---

EXAMPLE 1 — governance: true, technical: true
"The 'Relevant Policies' section lists only ETSI EN 319 401 (Clause 7.14). Please confirm whether you evaluated this incident against all other applicable requirements, including the S/MIME Baseline Requirements and Network Security Requirements."
→ Identifies a specific gap in the CA's policy analysis. technical: true.

EXAMPLE 2 — governance: true, technical: true
"Looking at https://crt.sh/?id=12345678 — the serialNumber field contains 64 bits of entropy but the leading byte is 0x00, making the effective entropy 56 bits. This violates BR 7.1 which requires 64 bits of output from a CSPRNG. How many certificates were issued with this encoding?"
→ Direct certificate field analysis with crt.sh link and specific BR citation. technical: true.

EXAMPLE 3 — governance: true, technical: false
"It has been over 14 days since the preliminary report. Where is the full incident report? Please note that the timer starts when you become aware, not when the preliminary report is published."
→ Governance: corrects CA's misunderstanding of timeline. But no technical content about the incident itself. technical: false.

EXAMPLE 4 — governance: true, technical: false
"Please open a new bug for the revocation failures and link it here. This bug should track only the misissuance."
→ Governance: legitimate process direction. But purely procedural — no analysis. technical: false.

EXAMPLE 5 — governance: true, technical: true
"Your root cause says 'software bug' but doesn't explain why pre-issuance linting didn't catch this. zlint flags exactly this encoding error. Was linting disabled for this certificate profile?"
→ Identifies a specific gap in root cause analysis referencing a known tool. technical: true.

EXAMPLE 6 — governance: false, technical: false
...
→ Pure process housekeeping. governance: false, technical: false.

EXAMPLE 7 — governance: false, technical: false
"Based on the facts described above, Swiss BIT is required to complete the incident reporting process set forth in the CCADB Incident Reporting Guidelines."
→ Boilerplate process notice. governance: false, technical: false.

EXAMPLE 8 — governance: true, technical: false
"Thanks for the detailed report. Can you confirm when the remediation will be fully deployed?"
→ Governance: substantive follow-up. But no technical analysis — just a timeline question. technical: false.

--- END EXAMPLES ---

Now classify the following comments. Return ONLY a valid JSON array, no other text.
...
def classify_comments_llm(candidates, existing_classifications, api_key):
    ...
    for i in range(0, len(needs), COMMENT_CLASSIFICATION_BATCH_SIZE):
        ...
        try:
            ...
            for item in items:
                ...
                if key:
                    gov = bool(item.get("governance", True))
                    existing_classifications[key] = {
                        "governance": gov,
                        # technical is only meaningful when governance=True
                        "technical": gov and bool(item.get("technical", False)),
                    }
                    classified += 1
            ...
            if e.code in (400, 401, 403):
                print(f"    Aborting remaining batches — fix the request before retrying")
                break
    ...
    if malformed_timestamps:
        import sys as _sys
        print(f"  WARNING: {malformed_timestamps} Bugzilla comments had malformed "
              f"creation_time and were excluded from quarterly oversight metrics.",
              file=_sys.stderr)
...
ROOT_PROGRAM_DOMAINS = {
    # chromium.org — Chrome root program infrastructure.
    # Nobody at GTS uses @chromium.org, so this is unambiguous.
    "chromium.org": "chrome",
    # Mozilla — @mozilla.org and @mozilla.com are root program domains.
    # Mozilla does not operate a publicly-trusted CA, so all Mozilla staff
    # comments are root program governance (no CA/RP conflation risk).
    "mozilla.com": "mozilla",
    "mozilla.org": "mozilla",
    # Apple — @apple.com; Apple does not operate a public CA.
    "apple.com": "apple",
    # Microsoft — @microsoft.com; self-incident exclusion handled in the
    # comment participation loop via _bug_is_about_program().
    "microsoft.com": "microsoft",
    # NOTE: google.com is intentionally absent. Google operates both the
    # Chrome root program AND Google Trust Services (a public CA). The
    # correct model is to attribute @google.com comments only when the
    # individual is a known Chrome root program staff member — the same
    # approach used for Ryan Sleevi's gmail. Named individuals are in
    # ROOT_PROGRAM_INDIVIDUALS below. All other @google.com comments
    # default to "other" (CA community peer commentary, not governance).
}
...
# OR who use @google.com (which is shared between Chrome RP and GTS).
...
ROOT_PROGRAM_INDIVIDUALS = {
    # ── Chrome root program ──────────────────────────────────────────────────
    # Ryan Sleevi: Chrome root program lead 2014–2022.
    # Used gmail throughout; consistently identified in public MDSP/CAB Forum record.
    "ryan.sleevi@gmail.com": "chrome",

    # Chrome root program team account — unambiguous; used for formal RP actions.
    "chrome-root-program@google.com": "chrome",

    # Ryan Dickson: Chrome root program lead post-Sleevi (~2022–present).
    # Confirmed via CA/B Forum attendance, MDSP posts, Chrome Security team page.
    "ryandickson@google.com": "chrome",

    # Devon O'Brien: Chrome root program manager ~2019–2022.
    # "asymmetric" is his Bugzilla handle. One recorded comment (DigiCert 2020)
    # clarifying Chrome's enforcement position — unambiguous root program action.
    "asymmetric@google.com": ("chrome", "2022-12-31"),

    # Emily Stark: leads Chrome's Secure Web and Network team, responsible for
    # the Chrome Root Program. Co-authored Symantec distrust post with Sleevi
    # and Devon O'Brien. Not yet in Bugzilla corpus but confirmed Chrome RP.
    "emily.stark@google.com": "chrome",
    "estark@google.com": "chrome",

    # David Adrian: PM on Chrome Security since 2022, focusing on PKI and the
    # Chrome Root Program. Present at F2F 65 (June 2025) as Chrome attendee.
    # Not yet in Bugzilla corpus.
    "dadrian@google.com": "chrome",

    # mozilla@google.com: 3 comments on ICP-Brasil bug (Nov-Dec 2024), asking
    # substantive compliance questions. Likely Chrome RP staff using a specific
    # email for Mozilla-adjacent work. Low confidence — leaving as chrome but
    # flagged for review if volume increases.
    "mozilla@google.com": "chrome",

    # Chris Clements: Chrome root program team (~2022–present).
    # Confirmed via CA/B Forum, MDSP posts, 66 comments across non-GTS CA bugs.
    "cclements@google.com": "chrome",

    # ── Mozilla root program ─────────────────────────────────────────────────
    # Gervase (Gerv) Markham: Mozilla root program engineer ~2003–2018.
    # Died February 2018. 178 other-CA comments in corpus (2014–2018).
    # Used gerv@mozilla.org on Bugzilla (not @mozilla.com — pre-dates that domain).
    # Already covered by ROOT_PROGRAM_DOMAINS ("mozilla.org": "mozilla") but
    # listed here explicitly with cutoff for documentation completeness.
    "gerv@mozilla.org": ("mozilla", "2018-02-28"),

    # Kathleen Wilson: Mozilla root program manager 2007–2019.
    "kathleen.a.wilson@gmail.com": ("mozilla", "2019-12-31"),

    # Wayne Thayer: Mozilla root program lead 2017–2020. Post-2020 joined Fastly
    # which operates Certainly CA — those comments are self-incident, not governance.
    "wthayer@fastly.com": ("mozilla", "2020-06-30"),
    "waynezilla@gmail.com": ("mozilla", "2020-06-30"),

    # Ben Wilson: Mozilla root program manager 2020–present.
    # 1,631 other-CA comments in corpus. @mozilla.com is already in
    # ROOT_PROGRAM_DOMAINS but listed here for documentation and explicit cutoff
    # tracking if his role ever changes.
    "bwilson@mozilla.com": "mozilla",

    # ── Microsoft root program ───────────────────────────────────────────────
    # Microsoft root program staff use @microsoft.com, which is in
    # ROOT_PROGRAM_DOMAINS — no explicit entries needed since Microsoft does not
    # operate a public CA (no CA/RP conflation risk unlike Google).
    # Named staff confirmed via CA/B Forum minutes (none appear in Bugzilla corpus
    # under @microsoft.com — Microsoft does not comment on other CAs' incidents):
    #   Karina Sirota (Goodley): root program lead ~2018–2023+
    #   Mike Reilly: root program ~2015–2020, now retired
    #   Dustin Hollenback: root program ~2018–2024, then moved to Apple
    #     → now appears as dhollenback@apple.com (32 comments, 2025-2026),
    #       correctly attributed to Apple via apple.com domain mapping
    # All are correctly attributed via domain fallback.
}
...
def classify_email(email, comment_time=""):
    ...
    Attribution model:
    - ROOT_PROGRAM_INDIVIDUALS: explicit named staff, checked first (highest confidence)
    - ROOT_PROGRAM_DOMAINS: unambiguous org domains (mozilla.org, apple.com, microsoft.com,
      chromium.org). google.com is intentionally absent — Google operates both Chrome root
      program and GTS CA. Named Chrome staff are in ROOT_PROGRAM_INDIVIDUALS instead.
    - Everything else: 'other'
    """
    if not email or "@" not in email:
        return "other"
    email_lower = email.lower().strip()
    # Named individuals checked first — highest confidence, supports time-bounded attribution
...
DISCOVERY_PATTERNS = {
    "self_detected": [
        ...
        # CA-authored incident report openers
        "this is the incident report", "this incident report",
        "preliminary incident report", "final incident report",
        "incident report for", "we are filing", "we are reporting",
        "we would like to report", "we want to report",
        "we hereby report", "we are writing to report",
        "this bug has been separated out",  # sub-CA split bugs, CA-authored
        "summary of incident", "timeline of incident",
        "problem summary", "this report covers",
        "we have recently", "we recently discovered", "we recently identified",
        "based on our", "in our review", "as part of our",
    ...
    "external_researcher": [
        ...
        # MDSP / public forum discovery
        "mozilla.dev.security.policy", "mdsp",
        "from https://groups.google.com", "groups.google.com/d/msg",
        "from the mozilla", "posted in the mozilla",
        "reported in the mozilla", "reported on the mozilla",
        "reported via", "user reported", "user noticed",
        "i found", "i noticed", "i discovered", "i identified",
        "i spotted", "i came across", "i was looking",
        "attached certificate", "created attachment",  # attachment = external reporter filing
    ],
    "root_program": [
        # Require context-specific phrases — bare names like "mozilla" or "google"
        # appear in CA-authored text constantly (policy references, URLs, etc.)
        # and would falsely override self_detected signals if matched alone.
        "root program", "root store",
        "reported by chrome", "reported by google", "chrome security team",
        "reported by mozilla", "mozilla root program", "mozilla root store",
        "reported by apple", "apple root program", "apple root store",
        "reported by microsoft", "microsoft root program", "microsoft root store",
        "notified by chrome", "notified by mozilla", "notified by apple",
        "chrome raised", "mozilla raised", "apple raised",
        "flagged by chrome", "flagged by mozilla", "flagged by apple",
        "browser vendor", "root program staff", "root program review",
        "per mozilla policy", "per chrome policy", "per apple policy",
        "in accordance with mozilla", "as required by mozilla",
        "as required by chrome", "as required by apple",
    ],
    "community": [
        "crt.sh", "censys", "zlint", "certlint",
        "ct log", "certificate transparency", "certificate transparency log",
        "publicly reported", "public discussion",
        "according to crt.sh", "as seen on crt.sh", "found on crt.sh",
        "ct monitor", "ct monitoring", "via ct", "in ct logs",
        "automated scan", "automated check", "automated detection",
        "linting tool", "lint tool", "pre-issuance lint",
        "pkilint", "x509lint", "dwklint",
        # community forum discovery (not researcher-specific)
        "bugzilla.mozilla.org/show_bug",  # link to another bug = community cross-reference
    ],
    "audit": [
        "audit found", "audit revealed", "auditor", "auditors",
        "webtrust", "etsi", "during audit", "audit report",
        "point-in-time", "period-of-time",
        "qualified auditor", "audit engagement", "audit period",
        "audit finding", "non-conformity", "qualified opinion",
        "annual audit", "compliance audit", "audit qualification",
        "during the audit", "identified by our auditor",
        "as noted by our auditor", "per our auditor",
        "management letter", "audit observation",
    ],
}
...
def classify_discovery_method(first_comment_text, _bug_creator, bug_summary):
    ...
    # Filter out misfiled browser bug reports — these have UA strings + reproduce steps
    # but NO certificate/PKI content. Only filter when UA string present + no PKI terms
    # in the first 300 chars (where UA template content lives)
    first_300 = text[:300]
    browser_markers = sum(1 for sig in ["user agent: mozilla/5.0", "applewebkit/537", "khtml, like gecko"] if sig in first_300)
    has_reproduce = "steps to reproduce" in first_300 or "actual results" in first_300
    pki_in_first = sum(1 for sig in ["certif", "issuan", "revoc", "ocsp", "crl ", "baseline"] if sig in first_300)
    if browser_markers >= 1 and has_reproduce and pki_in_first == 0:
        return "unknown"
...
def analyze_discovery_methods(bugs_raw, comment_cache):
    ...
    Filters to active-CA bugs only (excludes subsequently-distrusted CAs)
    to match the 1,428-bug corpus used throughout all other pipeline metrics.
    """
    import re as _re

    print("\n── Phase 1b: Discovery Method Classification ──")

    # Build distrusted CA name set — same filter logic as fetch_incidents.py
    mappings_path = PIPELINE_DIR / "bugzilla_ca_mappings.json"
    mappings_data = load_json(mappings_path, {})
    ca_mappings = mappings_data.get("mappings", {})
    distrusted_ca_names = {
        k.lower() for k, v in ca_mappings.items()
        if v.get("status") == "distrusted"
    }

    def _extract_ca(summary):
        if not summary or summary.startswith("[meta]"):
            return None
        m = _re.match(r'^([^:]+):', summary)
        if m:
            ca = _re.sub(r'^\[.*?\]\s*', '', m.group(1).strip())
            if 2 < len(ca) < 80:
                return ca
        return None

    def _is_distrusted_bug(bug):
        ca = _extract_ca(bug.get("summary", ""))
        return bool(ca and ca.lower() in distrusted_ca_names)
    ...
    for bug in bugs_raw:
        ...
        # Skip distrusted CA bugs — keep corpus consistent with incidents.json
        if _is_distrusted_bug(bug):
            excluded += 1
            continue
    ...
    print(f"  Classified {classified} active-CA bugs ({excluded} distrusted CA bugs excluded):")
    ...
    return {
        "discovery_methods": {
            ...
            "excluded_distrusted": excluded,
            "corpus_note": (
                f"{classified} bugs classified (excludes {excluded} distrusted-CA bugs). "
                f"Corpus is larger than the 1,428-incident count in incidents.json because "
                f"discovery classification runs on all non-distrusted bugs_raw entries, "
                f"including ~75 sub-operator naming variants (e.g. 'DigiCert / ABB') that "
                f"incidents.py normalizes into parent CA counts."
            ),
        }
    }
...
def _bug_is_about_program(summary, program):
    ...
    if program == "microsoft" and "microsoft" in s:
        return True
    if program == "chrome" and (
        "google trust services" in s or "gts" in s.split()
        or s.startswith("gts:")
    ):
        return True
...
def analyze_comment_participation(comment_cache, bugs_raw, comment_classifications=None):
    ...
    for bug_id, comments in comment_cache.items():
        ...
        for ci, comment in enumerate(comments):
            ...
            # Technical oversight — uses the LLM-classified "technical" field.
            # True = certificate/CRL analysis, specific BR citation with evidence,
            #        scope quantification, root cause identification.
            # False = process enforcement only (deadline notices, follow-up requests,
            #         housekeeping directives with no technical content).
            # Falls back to True when no classification is cached (conservative).
            # technical=True means the comment contains substantive findings
            # (cert/CRL analysis, specific BR citations with evidence, scope quantification).
            # When no classification is cached (LLM unavailable), default to False —
            # we don't know if it's substantive, so don't count it.
            # This means Substantive Oversight will be 0 without a populated cache,
            # making the empty-cache state visible rather than silently wrong.
            is_technical = clf_entry.get("technical", False) if clf_entry is not None else False
            if is_technical:
            ...
            if program not in seen_this_bug:
                ...
                # Bug 5 fix: only count oversight toward a year's coverage rate if the
                # comment was also made in that year. Retroactive comments on old bugs
                # were inflating historic coverage rates vs recent years.
                if comment_year == year:
                    by_year_oversight[year][program] += 1
                if comment_year >= RECENT_OVERSIGHT_CUTOFF:
                    unique_bugs[program]["recent_oversight"].add(bug_id)
                # Technical oversight tracks same set as oversight — any bug that
                # passed the governance filter counts. Must be inside seen_this_bug
                # guard so it matches the same denominator as oversight.
                if is_technical:
...
def compute_store_posture():
    ...
    incidents = load_json(OUTPUT_DIR / "incidents.json", {})
    
    # Build incident count lookup by CA owner
    inc_by_ca = {ca["ca"]: ca.get("n", 0) for ca in incidents.get("cas", [])}
    ...
    # ── Dark matter: CAs exclusive to a store with zero public incident record ──
    # These CAs are trusted but have never filed or received a public compliance
    # incident. They are invisible to the public governance record.
    STORES = ["chrome", "mozilla", "apple", "microsoft"]
    dark_matter = {s: {"exclusive_cas": 0, "exclusive_zero_incident": 0,
                       "zero_incident_cas": 0, "total_cas": 0} for s in STORES}

    for ca in market:
        tb = ca.get("trusted_by", {})
        stores_trusted = [s for s in STORES if tb.get(s)]
        if not stores_trusted:
            continue
        is_exclusive = len(stores_trusted) == 1
        has_incidents = inc_by_ca.get(ca["ca_owner"], 0) > 0

        for s in stores_trusted:
            dark_matter[s]["total_cas"] += 1
            if not has_incidents:
                dark_matter[s]["zero_incident_cas"] += 1
            if is_exclusive:
                dark_matter[s]["exclusive_cas"] += 1
                if not has_incidents:
                    dark_matter[s]["exclusive_zero_incident"] += 1

    for s in STORES:
        dm = dark_matter[s]
        excl_pct = round(dm["exclusive_zero_incident"] / max(dm["exclusive_cas"], 1) * 100, 1)
        zero_pct = round(dm["zero_incident_cas"] / max(dm["total_cas"], 1) * 100, 1)
        dm["exclusive_zero_pct"] = excl_pct
        dm["zero_incident_pct"] = zero_pct
        print(f"  {s:12}: {dm['total_cas']} CAs, {dm['zero_incident_cas']} ({zero_pct}%) zero-incident, "
              f"{dm['exclusive_cas']} exclusive, {dm['exclusive_zero_incident']} excl+zero ({excl_pct}%)")

    # ── Gov-affiliated CAs: read from curated gov_risk.json ──
    # gov_risk.json classifies each CA by structural ownership ties only.
    # Types "government" and "state_enterprise" are counted as gov-affiliated.
    # This replaces the prior inline GOV_PATTERNS string-match which had
    # both false positives (Certigna, Izenpe, Disig, Netlock classified as
    # commercial in the curated data) and false negatives (Deutsche Telekom,
    # HARICA, Telia, KIR, ANF AC missed by name patterns).
    gov_risk = load_json(OUTPUT_DIR / "gov_risk.json", {})
    gov_owners = {
        c["ca_owner"]
        for c in gov_risk.get("classified_cas", [])
        if c.get("type") in ("government", "state_enterprise")
    }
    if not gov_owners:
        print("  WARNING: gov_risk.json missing or empty — gov_ca_count will be 0",
    ...
    for store_name, store_data in intersections.get("per_store", {}).items():
        ...
        posture[store_key] = {
            ...
            "dark_matter": dark_matter.get(store_key, {}),
        }
...
def compute_policy_leadership():
    ...
    ballots_path = PIPELINE_DIR / "cabforum_ballots.json"
...
def compute_notable_gaps(enforcement_data):
    ...
    # Load pending Mozilla applications — these have actual filing dates
    inclusion_velocity = load_json(OUTPUT_DIR / "root_program_effectiveness.json", {})
    mozilla_pending_map = {}
    for entry in inclusion_velocity.get("inclusion_velocity", {}).get("mozilla_pending", []):
        ca_name = entry.get("ca", "")
        filed = entry.get("filed", "")
        if ca_name and filed:
            mozilla_pending_map[ca_name] = filed
    ...
    for ca in market:
        ...
        # Bug 6 fix: prefer Bugzilla application filing date over root creation date.
        # Root creation date may predate the actual application by years and produces
        # misleadingly large wait_years values for CAs that never applied to that store.
        wait_years = None
        wait_source = None
        if "mozilla" in missing_from and owner in mozilla_pending_map:
            try:
                filed_year = int(mozilla_pending_map[owner][:4])
                wait_years = now_year - filed_year
                wait_source = "bugzilla_filing"
            except (ValueError, IndexError):
                pass

        # Fall back to root creation date only when no filing date available,
        # and only when the CA is actually included somewhere (implying they did apply)
        if wait_years is None:
            root_date = oldest_root.get(owner, "")
            if root_date and root_date != "9999" and len(included_in) > 0:
                try:
                    root_year = int(root_date[:4])
                    wait_years = now_year - root_year
                    wait_source = "root_creation"  # less reliable
                except (ValueError, IndexError):
                    pass
...
def compute_inclusion_velocity():
    ...
    # Load trusted CA owners to distinguish new orgs from existing CAs adding roots
    trusted_owners = set()
    ...
    if market_path.exists():
        try:
            ...
            trusted_owners = {
                ca["ca_owner"].lower()
                for ca in market
                if any(ca.get("trusted_by", {}).values())
            }
        except Exception as e:
            # market_share.json is always present in normal operation.
            # If it fails here, classify_request() will treat every CA
            # as "new_org" rather than "existing_ca" — log so this is visible.
            print(f"  WARNING: failed to load market_share.json for inclusion velocity: {e}",
                  file=__import__("sys").stderr)

    # Stop-words that are too common to be meaningful for name matching
    _STOP = {"root", "cert", "trust", "security", "services", "authority",
             "certification", "digital", "national", "government", "global",
             "certificate", "ca", "ssl", "tls", "pki", "smime", "and", "the"}

    def classify_request(ca_name):
        """
        Returns ('new_org', None) or ('existing_ca', matched_owner).
        Existing CA = name shares meaningful non-stop words with a trusted owner.
        """
        name_words = {w for w in ca_name.lower().split() if len(w) > 3} - _STOP
        if not name_words:
            return "new_org", None
        for owner in trusted_owners:
            owner_words = {w for w in owner.split() if len(w) > 3} - _STOP
            overlap = name_words & owner_words
            if overlap:
                return "existing_ca", owner
        return "new_org", None
    ...
    try:
        ...
        for b in adds:
            ...
            req_type, matched = classify_request(b.get("summary", "").replace("Add ", ""))
            pending.append({
                "bug": b["id"],
                ...
                "days_waiting": days,
                "stage": stage,
                "request_type": req_type,   # "new_org" or "existing_ca"
            })
        ...
        for b in data.get("bugs", []):
            ...
            try:
                ...
                if days > 0 and c.year >= 2020:
                    req_type, _ = classify_request(b.get("summary", "").replace("Add ", ""))
                    completed.append({
                        "bug": b["id"],
                        "ca": b.get("summary", "").replace("Add ", "")[:60],
    ...
    # Stats — split by request type
    import statistics

    def make_stats(items, days_key):
        days_list = [x[days_key] for x in items]
        return {
            "count": len(items),
            "median_days": round(statistics.median(days_list)) if days_list else 0,
            "mean_days": round(statistics.mean(days_list)) if days_list else 0,
            "max_days": max(days_list) if days_list else 0,
        }

    completed_new = [c for c in completed if c["request_type"] == "new_org"]
    completed_exi = [c for c in completed if c["request_type"] == "existing_ca"]
    pending_new   = [p for p in pending   if p["request_type"] == "new_org"]
    pending_exi   = [p for p in pending   if p["request_type"] == "existing_ca"]
    ...
    stats = {
        ...
        # Split by type
        "new_org": {
            **make_stats(completed_new, "days"),
            "pending_count": len(pending_new),
            "longest_pending_days": max((p["days_waiting"] for p in pending_new), default=0),
        },
        "existing_ca": {
            **make_stats(completed_exi, "days"),
            "pending_count": len(pending_exi),
            "longest_pending_days": max((p["days_waiting"] for p in pending_exi), default=0),
        },
    }

    print(f"  Median completion (all): {stats['median_days']} days")
    print(f"  New orgs:     {len(pending_new)} pending, {len(completed_new)} completed, median {stats['new_org']['median_days']}d")
    print(f"  Existing CAs: {len(pending_exi)} pending, {len(completed_exi)} completed, median {stats['existing_ca']['median_days']}d")
...
def main():
    ...
    if max_fetch < len(bugs_raw):
        print(f"\n  WARNING: RPE_MAX_BUGS={max_fetch} — processing only {max_fetch}/{len(bugs_raw)} bugs")
