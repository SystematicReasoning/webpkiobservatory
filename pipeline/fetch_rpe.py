...
ROOT_PROGRAM_DOMAINS = {
    ...
    # Chrome root program AND reference entity (a public CA). The
    ...
    # approach used for public participant's gmail. Named individuals are in
...
# Public per-address role mappings are used only where a domain is ambiguous.
# Values may include a role cutoff date. Unknown or ambiguous contributors default
# to "other" rather than being inferred.
...
def mask_email(email):
    ...
    Example: account@example.com -> a******@example.com
    Applied to committed pipeline output to reduce email harvesting from
    public-record data.
    """
    if not email or "@" not in email:
        return email
...
def compute_enforcement_metrics():
    ...
    for e in raw_events:
        ...
        # reference entity and reference entity are the same event — combine
        ca_name = e.get("ca", "")
        if ca_name == "StartCom":
            continue
        if ca_name == "WoSign":
...
def compute_store_posture():
    ...
    # both false positives (reference entity, Izenpe, Disig, reference entity classified as
    # commercial in the curated data) and false negatives (Deutsche Telekom,
    # reference entity, reference entity, KIR, ANF AC missed by name patterns).
...
# People change jobs: public participant was reference entity then Mozilla. public participant was Mozilla then Fastly.
...
AMBIGUOUS_PEOPLE = {
    "ben wilson": {"mozilla": "mozilla", "digicert": None},      # reference entity era = CA, Mozilla era = browser
...
def _classify_participant(name):
    ...
    Org-aware: if someone changed jobs (e.g., public participant went from reference entity to Mozilla),
    ...
    # Check org names (catches "reference entity", "Mozilla Corporation", etc.)
