...
DIRECT_PDF_DOMAINS = {
    ...
    "matrix-tanusito.hu",
    "bmoattachments.org",
    "mozilla.org",          # bugzilla.mozilla.org attachment URLs (e.g. Certinomis)
    "bureauveritas.it",
    "actalis.it",
    "buypass.no",
    "webtrust.org",         # 503 currently but may recover
    "attestic.eu",          # Attestic B.V. (e.g. Buypass 2026)
}
...
def crosscheck_fingerprints(pdf_result, ccadb_roots_for_url,
    ...
    global_fp_type: dict fp->cert_type across ALL CCADB records — used to
                   ...
                   under a different CA owner (e.g. Certainly LLC under GoDaddy).
    """
    letter_fps = set(
...
def build_audit_profiles(ccadb_records, fetch_pdfs=True, max_pdfs=None,
    ...
    # it reflects post-acquisition ownership (e.g. Sectigo now owns Entrust
    # root records, so looking up an Entrust-signed cert's parent fp incorrectly
    # returns Sectigo). The CA Owner field on the intermediate record is set by
    ...
    for r in ccadb_records:
        ...
        # variants (D-TRUST→D-Trust, SECOM variants, etc.) so direct equality
        # is sufficient. Intra-org cross-signs (e.g. GoDaddy→Starfield,
        # Google Trust Services→GlobalSign) are naturally excluded because
    ...
    for ca, p in raw_profiles.items():
        ...
        profile = {
            ...
            # Used to suppress spurious scope gaps: if DigiCert has a Code Signing
