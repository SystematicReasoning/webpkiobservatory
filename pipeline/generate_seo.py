...
def generate_tab_pages(snap: dict, dist_dir: str):
    ...
    for tab_id, tab_label, tab_desc in tabs:
        ...
        desc = intro[:160].replace('"', '&quot;')
        full_title = f"WebPKI Observatory — {tab_label}"
        canonical = f"{SITE_URL}/#{tab_id}"
        html = f"""<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <meta property="og:site_name" content="WebPKI Observatory">
...
<body>
  <p><a href="{canonical}">WebPKI Observatory — {tab_label}</a></p>
  <p>{intro}</p>
