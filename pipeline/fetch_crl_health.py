...
def classify_url(url):
    ...
    if not errors:
        ...
        path = p.path
        if not path or path == "/":
            info.append("root_path")          # e.g. ISRG x1.c.lencr.org/ — valid
        else:
            last_seg = path.rstrip("/").split("/")[-1]
            if not last_seg:
                info.append("trailing_slash")
            elif "." not in last_seg:
                info.append("no_extension")   # e.g. Actalis getLastCRL — valid
            else:
                ext = last_seg.rsplit(".", 1)[-1].lower()
                if ext == "cgi":
                    info.append("cgi_endpoint")   # e.g. Netlock index.cgi?crl=gold — valid
